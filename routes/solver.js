...
fileSystem.stat(pythonFileName, function (err) {
  if (err == null) {
    ...
    console.log(
      "❌ ERROR: Solver python file does not exists: " + pythonFileName
    );
  }
});
...
router.post("/sendToQueue/:id", authenticateToken, async function (req, res) {
  ...
  try {
    ...
    fileSystem.stat(`excelFiles/uploaded/${fileId}.xlsx`, function (err) {
      if (err) {
        logger.error(
          `Send to queue scenario: Cannot find excelFiles/uploaded/${fileId}.xlsx`
        );
        return res.status(500).send("Cannot find file");
      }
    });
    const { rows } = await db.query(
      'SELECT * FROM public."Scenarios" WHERE id = $1',
      [fileId]
    );
    if (rows.length == 0) {
    ...
    if (role_id != 1 && role_id != 2) {
      if (rows[0].user_id != user_id) {
        logger.warn(
          `Send to queue Scenario #${fileId}: User #${user_id} tried to send scenario to queue when not allowed to`
        );
        return res
          .status(400)
          .send("Not allowed to send another user's scenario to queue");
      }
    }
    ...
    logger.info(
      `Scenario #${scenario_id} has been added to the queue: User #${user_id}`
    );
...
router.delete("/killSolver/:id", authenticateToken, async function (req, res) {
  ...
  try {
    ...
    if (req.role_id != 1 && req.role_id != 2) {
      ...
      if (rows.length == 0) {
        logger.error(
          `Kill solver scenario #${fileId}: Cannot find scenario in DB`
        );
        return res.status(500).send("Cannot find scenario in DB");
      }
      if (rows[0].user_id != req.user_id) {
        logger.warn(
          `Kill scenario #${fileId}: User #${user_id} tried to kill scenario when not allowed to`
        );
        return res
          .status(401)
          .json({
            success: false,
            message: "Not allowed to kill the current scenario",
          });
      }
    }
    await db.query(
      'UPDATE public."Scenarios" SET scenario_status = $1, error_message = $2 WHERE id = $3',
      [5, "Killed by user", fileId]
    );
    py.kill("SIGINT");
  ...
  try {
    if (req.role_id != 1 && req.role_id != 2) {
      ...
      if (rows.length == 0) {
        ...
        return res
          .status(500)
          .json({ success: true, message: "Cannot find scenario in DB" });
      }
      if (rows[0].user_id != req.user_id) {
        logger.warn(
          `Dequeue scenario #${fileId}: User #${user_id} tried to dequeue scenario when not allowed to`
        );
        return res
          .status(401)
          .send("Not allowed to dequeue another user's scenario");
      }
    }
    await db.query(
      'UPDATE public."Scenarios" SET scenario_status = $1 WHERE id = $2',
...
var solveScenario = async function (scenario_id, user_email, callback) {
  try {
    //Creating log file
    let writeStream = fileSystem.createWriteStream(
      `excelFiles/logs/${scenario_id}.txt`
    );
    writeStream.write(`### Scenario id: ${scenario_id} ###\n`);

    py = pySpawn("./venv/bin/python3", ["-u", pythonFileName, scenario_id]);
    await db.query(
      'UPDATE public."Scenarios" SET scenario_status = $1 WHERE id = $2',
      [1, scenario_id]
    );
    ...
    py.on("exit", async function (code) {
      ...
      const { rows } = await db.query(
        'SELECT scenario_status, error_message FROM public."Scenarios" WHERE id = $1',
        [scenario_id]
      );
      if (
        rows[0].scenario_status == 5 &&
        rows[0].error_message == "Killed by user"
      ) {
      ...
      if (
        rows[0].scenario_status == 5 &&
        rows[0].error_message == "Solution infeasible"
      ) {
      ...
      if (!errorEncountered) {
        ...
        await db.query(
          'UPDATE public."Scenarios" SET error_message = $1, scenario_status = $2 WHERE id = $3',
          [error, 5, scenario_id]
        );
...
setInterval(async () => {
  const { rows } = await db.query(
    'SELECT id, user_id FROM public."Scenarios" WHERE scenario_status = $1',
    [6]
  );

  const is_busy_query = await db.query(
    'SELECT is_busy FROM public."Solver_Busy" WHERE id = 1'
  );

...
const storage = multer.diskStorage({
  ...
  filename: async function (req, file, cb) {
    try {
      ...
      await db.query(
        'INSERT INTO public."TemplateFileInfo"(file_name) VALUES($1)',
        [file.originalname]
      );
      cb(null, "BGOInputFile.xlsx");
    } catch (err) {
      logger.error(
        `Admin #${req.user_id} changing excel template file: ${err}`
      );
    }
  },
});
...
router.delete("/deleteUser/:id", authenticateTokenAdmin, async (req, res) => {
  ...
  try {
    ...
    if (rows.length == 0) {
      ...
      const { rows } = await db.query(
        'DELETE FROM public."Users" WHERE id = $1 RETURNING email',
        [user_id]
      );
      const del_email = rows[0].email;
      logger.info(
        `Success deleting user #${user_id} - ${del_email}: Admin #${admin_id}`
      );
  ...
  async (req, res) => {
    ...
    try {
      if (new_role != 1 && new_role != 2 && new_role != 3) {
        logger.error(
          `Change user #${user_id} role: role ${new_role} is invalid`
        );
      ...
      if (rows.length == 0) {
        ...
        return res
          .status(400)
          .send(`User #${user_id} does not exist therefore cannot change role`);
      } else {
        await db.query('UPDATE public."Users" SET role_id = $1 WHERE id = $2', [
          new_role,
          user_id,
        ]);
        logger.info(
          `Changed role_id User #${user_id} to ${new_role}: Admin #${admin_id}`
        );
        return res.status(200).json({ success: true });
      }
    } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (rows.length == 0) {
        logger.error(
          `Admin change password: User #${user_id} does not exist in DB`
        );
        return res.status(400).send(`User #${user_id} does not exist`);
      } else {
        bcrypt.hash(new_password, saltRounds, async function (err, hash) {
          ...
          logger.info(
            `Admin changed password User #${user_id}: Admin #${admin_id}`
          );
      ...
      logger.error(
        `Admin #${admin_id} changing user #${user_id} password: ${err}`
      );
...
router.post("/createNewUser", authenticateTokenAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!checkEmail(email)) {
      return res
        .status(200)
        .json({ success: false, message: "Email invalid format" });
    }
    ...
    if (!checkRoleId(role_id)) {
      return res
        .status(200)
        .json({ success: false, message: "Role Id is invalid" });
    }
    ...
    if (!checkPassword(password)) {
      return res
        ...
        .json({
          success: false,
          message:
            "Password does not pass requirements (i.e Length > 8, Letters & Numbers included)",
        });
    }
    ...
    bcrypt.hash(password, saltRounds, async function (err, hash) {
      ...
      const { rows } = await db.query(
        `INSERT INTO public."Users"(name, role_id, password, email) VALUES($1, $2, $3, $4) RETURNING id`,
        [name, role_id, hash, email]
      );

      logger.info(
        `New User #${rows[0].id} role ${role_id}: Admin #${admin_id}`
      );
...
router.get("/getUsers", authenticateTokenAdmin, async (req, res) => {
  try {
    const { rows } = await db.query(
      'SELECT id, name, role_id, email FROM public."Users"'
    );
    return res.status(200).json(rows);
  } catch (err) {

<script>
  ...
  async function changeUserRole(id, og_role_id, new_role_id) {
    ...
    if (confirm(text)) {
      const res = await fetch(`/admin/changeUserRole/${id}/${new_role_id}`, {
        method: "POST",
      });
      const response = await res.json();
      if (response.success) {
  ...
  function promptAndDisableBtn(e) {
    if (
      confirm(
        `Are you sure you want to change the template file to ${templateFileName}`
      )
    ) {
      submitBtn.disabled = true;
      e.target.submit();
...
<form
  action="/admin/uploadTemplate"
  id="uploadForm"
  method="post"
  enctype="multipart/form-data"
  on:submit|preventDefault={promptAndDisableBtn}
>
  <div class="card shadow-lg form-container bg-accent-focus">
    <div class="p-3">
      <div class="flex justify-between">
        <p class="text-lg font-bold pb-3 underline">
          Upload & Change Template Excel File
        </p>
          ...
          <tr>
            ...
            <th>
              {#if user.role_id == 1}
                <select
                  ...
                  on:input={changeUserRole(user.id, 1, this.value)}
                >
              ...
              {:else if user.role_id == 2}
                <select
                  class="select select-bordered w-full max-w-xs"
                  on:input={changeUserRole(user.id, 2, this.value)}
                >
              ...
              {:else if user.role_id == 3}
                <select
                  class="select select-bordered w-full max-w-xs"
                  on:input={changeUserRole(user.id, 3, this.value)}
                >
                  <option value="1">Admin</option>
                  <option value="2">Super user</option>
            ...
            <th>
              ...
              >
                <button
                  class="btn btn-sm btn-error btn-circle btn-outline"
                  on:click={changePassword(user.id)}

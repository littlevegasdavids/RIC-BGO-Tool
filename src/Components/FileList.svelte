  ...
  $: {
    ...
    if (searchSelectValue == "User (Input File)") {
      ...
      if (user_id_filter != undefined) {
        fileReady = excelFileReady.filter((file) => {
          return file.user_id == user_id_filter;
        });
        fileBusy = excelFileBusy.filter((file) => {
          return file.user_id == user_id_filter;
        });
        fileSolved = excelFileSolved.filter((file) => {
          return file.user_id == user_id_filter;
        });
        fileError = excelFileError.filter((file) => {
          return file.user_id == user_id_filter;
        });
        fileQueue = excelFileQueue.filter((file) => {
          return file.user_id == user_id_filter;
        });
      }
    } else {
    ...
    if (search.trim() != "") {
      if (searchSelectValue == "File Name") {
        fileReady = excelFileReady.filter((file) => {
          return file.input_filename.toLowerCase().includes(search);
        });
        fileBusy = excelFileBusy.filter((file) => {
          return file.input_filename.toLowerCase().includes(search);
        });
        ...
        fileReady = excelFileReady.filter((file) => {
          return file.scenario_code.toLowerCase().includes(search);
        });
        fileBusy = excelFileBusy.filter((file) => {
          return file.scenario_code.toLowerCase().includes(search);
        });
        ...
        fileReady = excelFileReady.filter((file) => {
          return file.demand.toLowerCase().includes(search);
        });
        fileBusy = excelFileBusy.filter((file) => {
          return file.demand.toLowerCase().includes(search);
        });
...
<div class="flex flex-col">
  {#if excelFileReady.length != 0 || excelFileBusy.length != 0 || excelFileSolved.length != 0 || excelFileError.length != 0 || excelFileQueue != 0}
    <div class="form-control flex-row justify-center mt-5">
      ...
      <button
        ...
        }}>Reset<i class="fas fa-redo"></i></button
      >
    </div>
  {/if}
  ...
  {#if fileBusy.length != 0}
    <div
      class="collapse collapse-open collapse-arrow rounded-box shadow-lg form-container bg-accent p-3 mt-5"
      id="busyCard"
    >
      <input
        type="checkbox"
        on:input={() =>
          document.getElementById("busyCard").classList.remove("collapse-open")}
      />
  ...
  {#if fileQueue.length != 0}
    <div
      class="collapse collapse-open collapse-arrow rounded-box shadow-lg form-container bg-accent-content p-3 mt-5"
      id="queueCard"
    >
      <input
        type="checkbox"
        on:input={() =>
          document
            .getElementById("queueCard")
            .classList.remove("collapse-open")}
      />
      <div class="collapse-title text-xl font-bold text-black">Queue</div>
      <div class="collapse-content">
  ...
  {#if fileReady.length != 0}
    <div
      class="collapse collapse-open collapse-arrow rounded-box shadow-lg form-container bg-info p-3 mt-5"
      id="readyCard"
    >
      <input
        type="checkbox"
        on:input={() =>
          document
            ...
            .classList.remove("collapse-open")}
      />
      <div class="collapse-title text-xl font-bold text-black">Ready</div>
      <div class="collapse-content">
  ...
  {#if fileSolved.length != 0}
    <div
      class="collapse collapse-open collapse-arrow rounded-box shadow-lg form-container bg-primary p-3 mt-5"
      id="solvedCard"
    >
      <input
        type="checkbox"
        on:input={() =>
          document
            .getElementById("solvedCard")
            .classList.remove("collapse-open")}
      />
      <div class="collapse-title text-xl font-bold text-black">Solved</div>
      <div class="collapse-content">
  ...
  {#if fileError.length != 0}
    <div
      class="collapse collapse-open collapse-arrow rounded-box shadow-lg form-container bg-error p-3 mt-5"
      id="errorCard"
    >
      <input
        type="checkbox"
        on:input={() =>
          document
            ...
            .classList.remove("collapse-open")}
      />
      <div class="collapse-title text-xl font-bold text-black">Error</div>
      <div class="collapse-content">

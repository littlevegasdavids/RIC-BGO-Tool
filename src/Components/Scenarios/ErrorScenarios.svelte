<script>
    ...
    onMount(async ()=>{
        const res = await fetch('/scenarios/error')
        const result = await res.json()

        if(result.success){
            errorScenarios = result.scenarios
            finalScenarios = errorScenarios
        }
        else{
            alert('Error getting error scenarios from server')
        }
    ...
    function filterScenarios(){
        ...
        if(searchSelectValue == "User (Input File)"){
            ...
            if(user_id_filter != undefined){
                finalScenarios = errorScenarios.filter((file) => {return file.user_id == user_id_filter})
            }
        }
        else{
        ...
        if(search.trim() != ""){
            if(searchSelectValue == "File Name"){
                finalScenarios = errorScenarios.filter((file) => {return file.input_filename.toLowerCase().includes(search)})
            }
            else if (searchSelectValue == "Scenario Code"){
                finalScenarios = errorScenarios.filter((file) => {return file.scenario_code.toLowerCase().includes(search)})
            }
            else if (searchSelectValue == "Demand"){
                finalScenarios = errorScenarios.filter((file) => {return file.demand.toLowerCase().includes(search)})
            }
        }
        ...
        if(startDate != null && startDate != ""){
            ...
            if(endDate != null && endDate != ""){
                ...
                if(new Date(endDate) >= new Date(startDate)){
                    finalScenarios = errorScenarios.filter(file =>{
        ...
        if(filterSkuTypeValue == "Sku"){
            finalScenarios = errorScenarios.filter(file => file.sku_type == "sku")
        }
        else if(filterSkuTypeValue == "Grp"){
            finalScenarios = errorScenarios.filter(file => file.sku_type == "grp")
        }
    }
</script>
...
<div class="flex flex-col">
    ...
    <div class="tabs tabs-boxed justify-center mx-auto">
        ...
        <a class="tab tab-active" href="/error">Error</a>
    </div>

    {#if errorScenarios.length != 0}
        {#if finalScenarios.length != 0}
        <div class="card shadow-lg mt-5 bg-error">
            <div class="card-body">
        ...
        {/if}
    {:else}
        <p class="text-center font-bold text-2xl pt-10">Currently no error scenarios</p>
    {/if}

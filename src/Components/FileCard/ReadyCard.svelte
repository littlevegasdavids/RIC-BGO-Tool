<script>
    ...
    async function sendToQueue(){
        ...
        const res = await fetch(`/solver/sendToQueue/${fileId}`, {method: 'POST'})
        const result = await res.json()
    ...
    <div class="grid grid-cols-2 justify-items-stretch">
        ...
        <div class="justify-self-end">
            <div class="tooltip tooltip-bottom font-bold" data-tip="Send to Solver Queue">
                <button class="btn btn-sm btn-primary btn-outline btn-circle" id="solverButton" on:click={()=>sendToQueue()}><i class="fas fa-paper-plane"></i></button>
            </div>

<script>
    ...
    async function dequeueScenario(){
        const res = await fetch(`/solver/dequeueSolver/${fileId}`, {method: 'POST'})
    ...
    <div class="grid grid-cols-2 justify-items-stretch">
        ...
        <div class="justify-self-end">
            ...
            <div class="tooltip tooltip-bottom font-bold ml-3 mr-3" data-tip="Dequeue">
                <button class="btn btn-sm btn-error btn-circle btn-outline" on:click={()=>dequeueScenario()}><i class="fas fa-minus"></i></button>
            </div>
        </div>
    </div>
